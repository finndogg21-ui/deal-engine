...
**Last pass:** 2026-08-21

## Market snapshot (as of this pass)

- **Hidden Clearances** — free, scans 9,000+ stores across 7 retailers
  (Amazon, HD, Lowe's, Walmart, Costco, Target). Breadth over depth: a
  general scanner watching seven chains surfaces far fewer $0.01 finds per
  chain than a scanner pointed at one. This is the free alternative every
  prospect will compare us to first.
- **Deal Soldier** — ~$44/mo (some sources say $99/mo, pricing looks
  unstable/inconsistent across their own materials), HD/Lowe's/Walmart/Target,
  bundles reseller education content.
- **Scavenger** — ~$47/mo, penny-finder specialist, publishes competitive
  content ("7 best penny finder apps") — they're actively doing comparison
  SEO against us and everyone else.
- **Endless** — $9.99/mo, lower-end general clearance tool.
- **Rebel Savings** — free/low-cost community-sourced leads.
- Realistic paying market: ~30k–150k US penny/clearance hunters total, San
  Antonio's share is roughly 50–200 people. This is a tiny, winner-take-most
  local market, not a growth-at-all-costs SaaS market.

**Correction to my standing brief:** the brief I was given describes
deal-engine as "the priciest option at $75/mo." The actual code
(`src/vendors/stripe.ts`, `web/src/pages/site/Pricing.tsx`,
`web/src/pages/app/Welcome.tsx`) prices two tiers: **Consumer $9.99/mo**
and **Reseller $19/mo** (founding rate, capped at 30 seats, presumably
scheduled to rise later). At those prices deal-engine already undercuts
Deal Soldier and Scavenger and matches Endless — we are not the priciest
option, we're the cheapest *reseller-specialist* option. Positioning and
messaging should be rebuilt around that fact, not around competing on price
further down. (BUILD: if `$75/mo` shows up anywhere in code or copy, that's
a bug, not a strategy — flag it, don't ship it.)
...
- **Scan freshness / health tracking** — done, but *internal only*.
  ...
  Nothing about this is customer-facing yet — see Blueprint 2 below, which
  is the highest-leverage item in this backlog.
...
- **PR #1** (`company/night-2026-08-21` → `main`): wires the contact form
  to the existing `/api/contact` endpoint. Status: `in-pr`.

---

## Ranked backlog

### 1. Make the landing hero prove itself with real data
**status:** todo
...
the shelf" — is hardcoded copy, not a real finding. It never changes. A
visitor who scrolls past twice, or who is technical enough to view source,
sees a static prop dressed as live proof. For a product whose entire pitch
is "genuine machine-swept data, not stale scraped leads," a fake live
example is the single worst thing that could be sitting on the homepage —
it's the exact failure mode we accuse rivals of.
...
exact address, matching the existing card constraints already enforced
elsewhere) that returns the single most recent confirmed or high-confidence
penny find. Landing hero renders that when available; falls back to today's
static copy, clearly framed as an example, only when there's genuinely
nothing yet (new/quiet metro).
**why it beats today:** turns the hero from a claim into a demo. Every
competitor (including free Hidden Clearances) can write a compelling static
headline; only a real scanner can show you something that actually updates.
**effort:** M
...
### 2. Public live-scan freshness bar (the provable-freshness widget)
**status:** todo
**problem:** the single believable differentiator we have — genuine
machine-swept per-store stock counts with fresh timestamps — is invisible
to anyone who hasn't signed up. `Admin.tsx` already computes exactly this
(`last scan Xh ago`, rows written, stale flag) but it's gated behind the
admin route. Hidden Clearances' pitch is "9,000 stores, scanned 24/7" —
broad but unverifiable. We can out-verify them on our five stores instead
of trying to out-broad them.
...
red/stale treatment `Admin.tsx` already has for the internal version
(reuse the >26h stale threshold and copy pattern). No new backend logic,
just a public read of data that already exists.
**why it beats today:** turns "trust us, it's fresh" into a number a
skeptical reseller can watch update in real time — impossible for a rival
running on scraped/community leads to fake.
**effort:** M
...
The live scan covers Home Depot + Lowe's only — this was flagged in
`web/DESIGN_AUDIT.md` (P2-5) and explicitly left unfixed there as "a
marketing/roadmap copy decision, not a frontend defect... left for the
product owner." That's this backlog. Selling a plan on a retailer we don't
scan is a false-advertising / chargeback risk, not a style nit.
...
### 4. Answer "why pay when Hidden Clearances is free?" head-on
**status:** todo
**problem:** every prospect's first move is to check if a free tool
already does this. Hidden Clearances is a real, well-built free option and
Scavenger is already running comparison-SEO content ("7 best penny finder
apps") to win exactly this search. Our site currently has no page or FAQ
entry that engages this objection at all — `Faq.tsx` doesn't mention any
competitor.
**what to build:** one direct FAQ entry (or a short `/compare` page linked
from Pricing and FAQ) that names the tradeoff honestly: Hidden Clearances
covers 7 retailers broadly with community/scraped signal; deal-engine
covers 2 retailers deeply with a machine-swept, timestamped, per-store
scan and a score that's checked against confirmed outcomes weekly (already
true — see `src/engine/reputation.ts` / the "Members confirm" step in
`HowItWorks.tsx`). Own the "narrower but real" framing instead of avoiding
the comparison.
**why it beats today:** silence on the objection means the prospect answers
it themselves, usually by leaving. A confident, specific answer converts
skeptics that generic marketing copy can't reach — and doubles as SEO
content competing directly against Scavenger's comparison posts.
**effort:** S–M
...
out-of-scope for that pass: RetailerDeals `<img>` has no `onError`
fallback (shows a broken-image icon instead of the placeholder AllDeals
already uses), and a handful of dead CSS rules
...
already cheaper than the two direct reseller-specialist rivals), the
marketing thesis should shift from "worth paying more for" to "the
cheapest tool that's actually verified fresh" — a materially different
story than what a $75/mo positioning would need. This touches Landing,
Pricing, and How It Works copy simultaneously and should be scoped as its
own review rather than folded into the smaller copy fixes above.
...
worth your gas money," which none of the four named competitors do. Needs
a resale-comp data source decision before this can be scoped as buildable
PRs.
